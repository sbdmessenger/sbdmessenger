...
var lastOne = '0';
var chunk = 1000;
var FollowersName = 'dollarvigilante'; //joanaltres dantheman always1success cryptoriddler dollarvigilante imagediet jerrybanfield carlobelgado sweetsssj alphacore kingscrown
var myAccount = 'sbdmessenger';
var myMemo = 'MSG@github';
...
function onInactiveFollowersRemoved() {
   ...
   appendHTML('<b>Active Users: </b>' + followers__.length, true);
   appendHTML('<b>Total Costs: </b>' + totalFee.toFixed(3) + ' SBD');
   stopLoading();
   disableAll(false);
}
...
//   document.getElementById('FollowersName').disabled = flag;
...
function transferTheTips() {
   ...
   var confirm_ = confirm("This is a clientside application, which means if you close this window, the application will stop without completeing all the transactions. The first transaction will be @sbdmessenger's fee which is " + myFee_ + ". All transfers are non-refundable. Are you sure you want to continue?");
   ...
   if (confirm_) {
      ...
      steem.broadcast.transfer(activeKey, accountName, myAccount, myFee_, myMemo, function (err, result) {
         if (err == null) {
            appendHTML("Transfered " + myFee_ + " To @sbdmessenger.");
            var activeKey = document.getElementById('activeKey').value;
            var accountName = document.getElementById('accountName').value;
            var tipMessage = document.getElementById('tipMessage').value;
            var tipAmount = document.getElementById('tipAmount').value;
            ...
            appendHTML("Transfering " + myFee_ + " To @sbdmessenger Failed.");
            disableAll(false);
            console.log(err);
            stopLoading();
         }
